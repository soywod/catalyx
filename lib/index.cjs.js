...
function trexFactory(e) {
    ...
    var bindings = {
        ...
        bindList: function (obs$, render) {
            elems.forEach(function (elem) {
                var prev$ = new rxjs.BehaviorSubject([]);
                var subscription = obs$
                    .pipe(operators.withLatestFrom(prev$), operators.flatMap(function (_a) {
                    var next = _a[0], prev = _a[1];
                    return [
                        arrayDiffs(prev, next, isEqual),
                        rxjs.defer(function () { return prev$.next(Object.assign([], next)); }),
                    ];
                }), operators.mergeAll())
